...
(function($){
	...
	function Slider(container, settings){
		...
		this.container.bind('everslider', function(e, method){	
	...
	Slider.prototype.slideOffset = function(offset){
		// do nothing if offset won't change
		if(offset === this.offset) return;
		
		var self = this, adjustment = false;
		var unlock = function(){
			// unlock slides
			self.lock = false;
			// save new offset
		...
		// if this.offset is "-1" then it's an adjustment, not actual 'slide' animation
		if(this.offset === -1) {
			adjustment = true;
		}
		
		// get pixels by offset
		var pixel_offset = offset * (this.slide.width() + this.slide_margin);
								
		// code below performs carousel animation using selected effect; code is split in two
		// blocks - one if browser is CSS3 compliant, the other one is a fallback to jQuery $.animate
		if( this.cssok ) {
			// 'fade' animation effect
			if(this.settings.effect === 'fade' && !adjustment) {
				...
				// 'slide' animation effector or adjustment
			...
			if(this.settings.effect === 'fade' && !adjustment) {
				...
				// 'slide' animation effect or adjustment (jQuery fallback)
		...
		if(!adjustment && typeof this.settings.beforeSlide === 'function') {
	...
	Slider.prototype.enableTouchSwipe = function(){
		...
		var swipeMove = function(e) {
			...
			if( !self.settings.swipePage || swipe_x > swipe_y ) {
				e.preventDefault();
			}
			...
			if(Math.abs(swipe_x) < self.settings.swipeThreshold) {
				...
				// get swipe direction and move slides
				var swipe_direction = (swipe_x > 0) ? 'prev' : 'next';
				var offset = self.getOffset(swipe_direction);
				// when getOffset will start giving the same number, fake offset 
				// to trigger slideOffset() to return list to the place
				self.offset = (offset === self.offset) ? -1 : self.offset;
				self.slideOffset(offset);
				// remove move handler
		...
		var swipeEnd = function(){
			if( swipe ){
				// correct offset on touchend if needed
				if(!self.lock && pixel_offset !== getPixelOffset(self.list, self.cssok).left) {
					var offset = self.offset;
					// fake offset to trigger slideOffset() with the same offset number
					self.offset = -1;
					self.slideOffset(offset);
				}
				// unbind move handler
		...
		// disable dragging
		this.container.find('a').bind('mousedown', function(e){
			e.preventDefault();
		});
	};
	...
	$.fn.everslider = function(c) {
		...
		var s = $.extend({
			...
			swipePage: false,					// whether to allow vertical swipe to scroll page
